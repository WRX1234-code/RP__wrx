...
static void Auto_Rescue_Target_Process(Chassis_t* My_Chassis);
static void Ctrl_Rescue_Target_Process(Chassis_t* My_Chassis);
...
static void Pitch_Detect(Chassis_t* My_Chassis);
...
Chassis_Target_t Chassis_Target = 
{
	...
	.yaw = 0.f,
	.roll = 0.f,
	.thetal_r= THETAL_OFFSET,
	.thetal_l= THETAL_OFFSET,
//	.thetal_r=0.f,
//	.thetal_l=0.f,
...
/**
  ...
  * @retval None
  */
float takeoff_x_speed = 0;
float takeoff_y_speed = 0;
static void Chassis_State_Var_Update(Chassis_t* My_Chassis)//角度均用弧度制
{
	...
	R_Leg_State_Var->thetal = (R_VIR_PHI0_ORDER_CORRECT*R_Leg_Link->info->angle->vir_phi0 - (My_Posture->pitch + 0.012f));
	...
	L_Leg_State_Var->thetal = (L_VIR_PHI0_ORDER_CORRECT*L_Leg_Link->info->angle->vir_phi0 - (My_Posture->pitch + 0.012f));
...
//	My_State_Var->sd1 = 0.5f * WHEEL_RADIUS * (-(float)L_Wheel.rx_info->speed \
	...
	if(My_Chassis->Leg_Unit[R_Leg]->off_ground_cnt == 0 && My_Chassis->Leg_Unit[L_Leg]->off_ground_cnt == 0)
	{
		takeoff_x_speed = My_filter_S_d1;
	}
...
static void Test_phi0_l0_Ctrl(Chassis_t *My_Chassis)
{
	...
	My_Chassis->target->vir_phi0_r +=My_Chassis->rc_input->ch3_now/660.f*TIME_STEP * 10000;
	My_Chassis->target->vir_phi0_l +=My_Chassis->rc_input->ch3_now/660.f*TIME_STEP * 10000;
...
static void Chassis_Status_React(Chassis_t *My_Chassis)
{
  ...
  if(Balance.Flag->Fly_Flag == true || Balance.Flag->Reserve_Fly_Flag == true)
  {
	  ...
	  #else
		  if(Balance.Flag->Rescue_Flag == true)// || Balance.Flag->Ctrl_Rescue_Flag == true
		  {
			  My_Chassis->mode = C_Rescue;
...
//		  Test_Straight_Ctrl(My_Chassis);
		...
		case C_Rescue:
			Auto_Rescue_Target_Process(My_Chassis);
		   Chassis_Set_Torque(My_Chassis);
		   break;
...
//			}	
				...
				
				{
					...
					My_Chassis->chassis_PID->length_cal[L_Leg]->kp=jump_info->IDLE_length_l_kp;
					My_Chassis->chassis_PID->length_speed_cal[L_Leg]->kp=jump_info->IDLE_length_l_speed_kp;
					My_Chassis->chassis_PID->length_cal[R_Leg]->out_max=jump_info->IDLE_length_r_outmax;
					My_Chassis->chassis_PID->length_speed_cal[R_Leg]->out_max=jump_info->IDLE_length_r_speed_outmax;
...
static void Auto_Rescue_Target_Process(Chassis_t* My_Chassis)
{
	static bool gimbal_ok = false;
	static bool last_ctrl_rescue = false;
	...
	last_ctrl_rescue = Balance.Flag->Ctrl_Rescue_Flag;
//	if(Balance.Flag->Ctrl_Rescue_Flag == true)
//	{
//		Ctrl_Rescue_Target_Process(My_Chassis);
//		return;
//	}
//	else{
		...
		switch (rescue_info->state)
  	{
	      ...
	      My_Chassis->target->leg_length_r = 0.13f;
	        ...
	        My_Chassis->target->leg_length_r = TAR_LEG_LENGTH_INITIAL;
					...
					My_Chassis->rescue_info->must_restrict = false;
		      Balance.Flag->Rescue_OK = true;
...
static void Ctrl_Rescue_Target_Process(Chassis_t* My_Chassis)
{
	static float R_ang_tar = 0,R_len_tar = 0,L_ang_tar = 0,L_len_tar = 0;
	...
	if(first_flag == true)
	{
		R_ang_tar = My_R_Link->info->angle->vir_phi0_;
		L_ang_tar = My_L_Link->info->angle->vir_phi0_;
		
		R_len_tar = My_R_Link->info->length->l0;
		L_len_tar = My_L_Link->info->length->l0;
		
		first_flag = false;
	}
	
	R_ang_tar -= (float)(rc_sensor.info->Q.cnt - rc_sensor.info->W.cnt)/(float)KEY_Q_CNT_MAX * 0.2f;
	L_ang_tar -= (float)(rc_sensor.info->R.cnt - rc_sensor.info->E.cnt)/(float)KEY_R_CNT_MAX * 0.2f;
	R_len_tar -= (float)(rc_sensor.info->A.cnt - rc_sensor.info->S.cnt)/(float)KEY_A_CNT_MAX * 0.0002f;
	L_len_tar -= (float)(rc_sensor.info->F.cnt - rc_sensor.info->D.cnt)/(float)KEY_F_CNT_MAX * 0.0002f;
	
	if(fabs(R_ang_tar) > 180.f)
	{
		R_ang_tar -= sgn(R_ang_tar) * 360.f;
	}
	if(fabs(L_ang_tar) > 180.f)
	{
		L_ang_tar -= sgn(L_ang_tar) * 360.f;
	}
	
	R_len_tar = constrain(R_len_tar,MIN_LEG_LENGTH,MAX_LEG_LENGTH);
	L_len_tar = constrain(L_len_tar,MIN_LEG_LENGTH,MAX_LEG_LENGTH);
	
	/*摆角控制*/
	My_Chassis->target->vir_phi0_r = R_ang_tar;
	My_Chassis->target->vir_phi0_l = L_ang_tar;
	Chassis_Leg_vir_phi0_Cal(My_Chassis);
		
	/*腿长控制*/
	My_Chassis->target->leg_length_r = R_len_tar;
	My_Chassis->target->leg_length_l = L_len_tar;
	Chassis_Leg_Length_Strength_Cal(My_Chassis);
...
//	                                R_Link_info->length->l0*powf(Leg_info->thetald1_l, 2)*arm_cos_f32(Leg_info->thetal_l));
  ...
  My_Chassis->Leg_Unit[L_Leg]->force->F_sup1 = L_Link_info->force->F_bl_mea * cos(L_Straight_info->thetal);
	My_Chassis->Leg_Unit[L_Leg]->force->F_sup2 = m_l*My_Chassis->Posture->info->z_world;
	My_Chassis->Leg_Unit[L_Leg]->force->F_sup3 = m_l*((1.f - L_Link_info->centroid->centriod_coefficient) * L_Link_info->length->l0_dot2 * cos(L_Straight_info->thetal));
	
	My_Chassis->Leg_Unit[R_Leg]->force->F_sup1 = R_Link_info->force->F_bl_mea * cos(R_Straight_info->thetal);
	My_Chassis->Leg_Unit[R_Leg]->force->F_sup2 = m_l*My_Chassis->Posture->info->z_world;
	My_Chassis->Leg_Unit[R_Leg]->force->F_sup3 = m_l*((1.f - R_Link_info->centroid->centriod_coefficient) * R_Link_info->length->l0_dot2 * cos(R_Straight_info->thetal));
...
//	My_Chassis->chassis_PID->length_cal[L_Leg]->target = My_Chassis->target->leg_length_l;
	...
	My_Chassis->chassis_PID->length_cal[L_Leg]->target = My_Chassis->target->leg_length_l + My_Chassis->Leg_Unit[L_Leg]->force->F_roll;
...
//			+ My_Chassis->Leg_Unit[L_Leg]->force->F_roll
	   ...
	   else if (Balance.Flag->Jumping_Flag==false&&My_Chassis->Leg_Unit[R_Leg]->off_ground == true&&My_Chassis->Leg_Unit[L_Leg]->off_ground ==false)
	   {
		 ...
		 else if(Balance.Flag->Jumping_Flag==false&&My_Chassis->Leg_Unit[R_Leg]->off_ground == false&&My_Chassis->Leg_Unit[L_Leg]->off_ground ==true)
		 {
			 My_Chassis->Leg_Unit[R_Leg]->force->F_bl_target = My_Chassis->Leg_Unit[R_Leg]->force->F + My_Chassis->Leg_Unit[R_Leg]->force->F_gravity
			  							 			  	                         + My_Chassis->Leg_Unit[R_Leg]->force->F_inertial;
			 My_Chassis->Leg_Unit[L_Leg]->force->F_bl_target = My_Chassis->Leg_Unit[L_Leg]->force->F;//+ My_Chassis->Leg_Unit[L_Leg]->force->F_gravity
...
static void Chassis_Roll_Control(Chassis_t* My_Chassis)
{
	static float last_roll_r = 0,last_roll_l = 0;
	...
	last_roll_r = My_Chassis->Leg_Unit[R_Leg]->force->F_roll;
  last_roll_l = My_Chassis->Leg_Unit[L_Leg]->force->F_roll;
...
static void Chassis_Yaw_Target_Process_All(Chassis_t* My_Chassis)
{
    ...
    case C_Rescue:
		...
		case C_Follow:
			if(Balance.Flag->U_Turn_Flag == false)
			{
				My_Chassis->target->yaw_v = angle2rad(gimbal.cmd.yaw_imu_tar - last_yaw_imu) /TIME_STEP;
			}
			else{
			  My_Chassis->target->yaw_v = 0;
			}
...
//	} 
	...
	{
		if(My_Chassis->mode != C_Follow)
		{
			My_Chassis->target->yaw += My_Chassis->target->yaw_v*TIME_STEP;
		}
		
		if(fabs(half_cycle(My_Chassis->target->yaw - My_Chassis->Posture->info->yaw,2*PI)) >= 3.f)
		{
			My_Chassis->target->yaw = sgn(half_cycle(My_Chassis->target->yaw - My_Chassis->Posture->info->yaw,2*PI)) *3.f + My_Chassis->Posture->info->yaw;
		}
	...
	last_yaw_imu = gimbal.cmd.yaw_imu_tar;
}
/**
  * @brief  赋予左右腿腿长目标值
  * @param  Chassis_t* My_Chassis
...
static void Chassis_Rc_Input_Update(Chassis_t* My_Chassis)
{
	...
	rc_input->ch2_now=step_limit_filter(rc_input->ch2_now,rc_input->ch2_last,5);
	...
	if(My_Chassis->Leg_Unit[R_Leg]->off_ground == true || My_Chassis->Leg_Unit[L_Leg]->off_ground == true)
	{
		...
		{
			rc_input->ch3_last = (int16_t)(takeoff_x_speed*660.f/My_Chassis->target->velocity_max);
		}
...
//	if(fabs(My_Chassis->target->sd1 * My_Chassis->Posture->info->yaw_v) >= MAX_STRAIGHT_SPEED * MAX_LIFT_SPEED/3)
//	{
//		My_Chassis->target->velocity_max = MAX_STRAIGHT_SPEED * MAX_LIFT_SPEED/3/fabs(My_Chassis->Posture->info->yaw_v);
//	}
	if(Balance.Flag->Fly_Flag == true || Balance.Flag->Reserve_Fly_Flag == true)
	{
...
//		My_Chassis->target->velocity_max = 1.7f;
	...
	if(end_power == 0)
	{
	  ...
	  else
	  {
	    ...
	    {
	      My_Chassis->target->sd1 = head_to * KEY_INPUT_SD1_ORDER_CORRECT *((float)My_Chassis->rc_input->w_s_now) /(float)rc_sensor.info->S.cnt_max * My_Chassis->target->velocity_limit;
	...
	else if(end_power == 1)
	{
	  ...
	  else
	  {
	    ...
	    {
	      My_Chassis->target->sd1 = head_to * KEY_INPUT_SD1_ORDER_CORRECT *((float)My_Chassis->rc_input->w_s_now) /(float)rc_sensor.info->S.cnt_max * My_Chassis->target->velocity_max;
...
//	Chassis_Power_Limit(My_Chassis);
	...
	else if((float)fabs(My_Chassis->target->sd1 / MAX_STRAIGHT_SPEED) >= 0.1f)// || (float)fabs(My_Chassis->Leg_Unit[R_Leg]->Straight->info->sd1 / MAX_STRAIGHT_SPEED) >= 0.1f
	{
		if(Balance.Flag->Fly_Flag == false)
		{
			My_Chassis->target->thetal_r = 0.f;
	...
	else if((float)fabs(My_Chassis->target->sd1 / MAX_STRAIGHT_SPEED) < 0.1f)
	{
		My_Chassis->target->thetal_r = THETAL_OFFSET;
		My_Chassis->target->thetal_l = THETAL_OFFSET;
	...
	else if(My_Chassis->mode == C_Turn)
	{
		My_Chassis->target->thetal_r = THETAL_OFFSET;
		My_Chassis->target->thetal_l = THETAL_OFFSET;
		stop_tick = 0;
	...
	else if( My_Chassis->mode == C_Knee_Strike || My_Chassis->mode == C_Jump)
	{
		My_Chassis->target->thetal_r = 0.f;
		My_Chassis->target->thetal_l = 0.f;
		stop_tick = 0;
...
static void My_Chassis_KEY_Input(void)
{
	...
	{
		rc_input->w_now = D_Board_Tx_Pkt.v_x*KEY_W_CNT_MAX/(float)Chassis.target->velocity_max;
	}
	...
	{
		rc_input->s_now = -D_Board_Tx_Pkt.v_x*KEY_S_CNT_MAX/(float)Chassis.target->velocity_max;
	}
	...
	{
		rc_input->d_now = -D_Board_Tx_Pkt.v_y*KEY_D_CNT_MAX/1.8f;
	}
	...
	{
		rc_input->a_now = -D_Board_Tx_Pkt.v_y*KEY_A_CNT_MAX/1.8f;
	}
	...
	rc_input->a_now = Lowpass(rc_input->a_last,rc_input->a_now, 0.4f);
	rc_input->a_last = rc_input->a_now;
	
	//腿长
	//平移
	rc_input->d_now = Lowpass(rc_input->d_last,rc_input->d_now, 0.4f);
	rc_input->d_last = rc_input->d_now;
	
	
	rc_input->w_s_now = rc_input->w_now - rc_input->s_now;
	rc_input->w_s_now = step_limit_filter(rc_input->w_s_now,rc_input->w_s_last,5);
	
	if(Chassis.Leg_Unit[R_Leg]->off_ground == true && Chassis.Leg_Unit[L_Leg]->off_ground == true)
	{
		if(gimbal.cmd.yaw_mec_tar == gimbal.info.cfg_info.head_to[0])
		{
			rc_input->w_s_last = D_Board_Tx_Pkt.v_x*KEY_W_CNT_MAX/(float)Chassis.target->velocity_max;
		}
		else if(gimbal.cmd.yaw_mec_tar == gimbal.info.cfg_info.head_to[4])
		{
			rc_input->w_s_last = -D_Board_Tx_Pkt.v_x*KEY_W_CNT_MAX/(float)Chassis.target->velocity_max;
	  ...
	  rc_input->w_s_last = rc_input->w_s_now;
	}
	
	rc_input->d_a_now = rc_input->d_now - rc_input->a_now;
	rc_input->d_a_now = step_limit_filter(rc_input->d_a_now,rc_input->d_a_last,5);
	
	if(Chassis.Leg_Unit[R_Leg]->off_ground == true && Chassis.Leg_Unit[L_Leg]->off_ground == true)
	{
		if(gimbal.cmd.yaw_mec_tar == gimbal.info.cfg_info.head_to[0])
		{
			rc_input->d_a_last = -D_Board_Tx_Pkt.v_y*KEY_A_CNT_MAX/1.8f;
		}
		else if(gimbal.cmd.yaw_mec_tar == gimbal.info.cfg_info.head_to[4])
		{
			rc_input->d_a_last = D_Board_Tx_Pkt.v_y*KEY_A_CNT_MAX/1.8f;
	  ...
	  rc_input->d_a_last = rc_input->d_a_now;
	}
...
//	T_comp = -263.015436*L^3 + 356.592109*L^2 + -147.996968*L + 17.562143
//  T_comp = -26.288272*L^3 + 191.911820*L^2 + -108.590470*L + 15.081256
	
//	T_comp = -656.638423*L^3 + 594.118436*L^2 + -191.242242*L + 20.553452
...
float Chassis_S_Turn_sdl_update(Chassis_t* My_Chassis)
{
	...
	{
		tar_x = 1.f * ((float)My_Chassis->rc_input->w_s_now)/ (float)rc_sensor.info->S.cnt_max* 1.8f;
		tar_y = 1.f * ((float)My_Chassis->rc_input->d_a_now)/ (float)rc_sensor.info->A.cnt_max* 1.8f;
...
static void Pitch_Detect(Chassis_t* My_Chassis)
{
	static bool pitch_wrong = false;
	if(fabs(My_Chassis->Posture->info->pitch) >= 15.f/180.f*PI && pitch_wrong == false)
	{
