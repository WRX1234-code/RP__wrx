...
enum judge_std_length_t {
	/* Std */
	LEN_FRAME_HEAD 	                 = 5,	// 帧头长度：SOF1 data_length2 seq包序号1, CRC81
	LEN_CMD_ID 		                 = 2,	// 命令码长度
...
typedef __packed struct 
{
 ...
 uint8_t cooling_buff; //热量冷却增益
 uint8_t defence_buff; //防御增益
 uint8_t vulnerability_buff; //负防御增益
 uint16_t attack_buff; //攻击增益
  __packed union 
	{
		uint8_t remaining_energy;  //剩余底盘能量
		__packed struct//低位优先，bit0先发
		{
			
			uint8_t  over_50_percent :1;
			uint8_t  over_30_percent :1;
			uint8_t  over_15_percent :1;
			uint8_t  over_5_percent :1;
			uint8_t  over_1_percent :1;
			uint8_t  keepnull :3;
			
		}bit;
	}remaining_energy;//剩余底盘能量
}ext_buff_t;
...
typedef __packed struct 
{
	uint8_t armor_id;// 被击打的装甲板的ID

	uint8_t rx_flag; // 接受标志，接收到置1，在处理完UI后会置0
}armor_hurt_state_t;
...
{
	uint8_t offline_cnt;
	dev_work_state_t user_client_state;
} user_client_state_t;
...
typedef struct {
	...
	ext_remote_control_t            remote_control;            // 0x0304			
	
	ext_interact_id_t								ids;								       //与本机交互的机器人id
	       
	uint16_t                        self_client;               //本机客户端
	
	dev_work_state_t judge_work_state;
