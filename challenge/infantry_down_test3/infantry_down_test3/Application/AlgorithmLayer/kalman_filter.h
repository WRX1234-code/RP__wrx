...
typedef struct kf_t
{
    ...
    // definiion of struct mat: rows & cols & pointer to vars,矩阵结构体的变量名
    mat xhat;      // x(k|k)后验状态估计矩阵
    mat xhatminus; // x(k|k-1)先验状态估计矩阵
    mat u;         // control vector u输入向量矩阵
    mat z;         // measurement vector z观测向量矩阵
    mat P;         // covariance matrix P(k|k)后验状态估计误差协方差矩阵
    mat Pminus;    // covariance matrix P(k|k-1)先验状态估计误差协方差矩阵
    mat F, FT;     // state transition matrix F FT状态转移矩阵及其逆矩阵
    mat B;         // control matrix B输入矩阵
    mat H, HT;     // measurement matrix H观测矩阵
    mat Q;         // process noise covariance matrix Q过程噪声矩阵
    mat R;         // measurement noise covariance matrix R测量噪声矩阵
